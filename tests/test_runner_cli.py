    ...
    def test_build_uses_build_for_testing_action(self):
        ...
        for cmd in all_cmds:
            if "xcodebuild" in cmd and "build-for-testing" in cmd:
                return
        pytest.fail(f"Expected 'build-for-testing' action in xcodebuild call. Cmds: {all_cmds}")

    def test_build_uses_iphonesimulator_sdk(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")
            _invoke("runner", "build")
        all_args = " ".join(str(a) for c in mock_run.call_args_list if c[0] for a in c[0][0])
        assert "iphonesimulator" in all_args.lower(), f"xcodebuild must target iphonesimulator SDK. Args: {all_args}"
...
class TestRunnerStatusBuilt:
    ...
    def test_status_reports_built_when_binary_exists(self, tmp_path):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            result = CliRunner().invoke(ios_command_group, ["runner", "status"], env=env)
...
class TestRunnerStatusNotBuilt:
    ...
    def test_status_suggests_build_command(self, tmp_path):
        ...
        assert "build" in output.lower(), f"Status must suggest running 'runner build'. Got: {output!r}"
...
class TestRunnerClean:
    ...
    def test_clean_removes_derived_data(self, tmp_path):
        ...
        with patch("subprocess.run", return_value=MagicMock(returncode=0)):
            ...
            result = CliRunner().invoke(ios_command_group, ["runner", "clean", "--yes"], env=env)
    ...
    def test_clean_tolerates_missing_directory(self, tmp_path):
        ...
        with patch("subprocess.run", return_value=MagicMock(returncode=0)):
            ...
            result = CliRunner().invoke(ios_command_group, ["runner", "clean", "--yes"], env=env, catch_exceptions=True)
        assert result.exit_code == 0, (
            f"clean should tolerate missing dir. Exit={result.exit_code}. Output: {result.output}"
        )
...
class TestRunWithRunner:
    ...
    def test_run_with_runner_flag_accepted(self):
        ...
        assert "No such option" not in (result.output or ""), f"--runner flag not accepted. Output: {result.output}"
    ...
    def test_run_with_runner_uses_xctest_backend(self):
        ...
        if mock_som_cls.called:
            ...
            if runner_url:
                assert "8222" in runner_url, f"--runner flag must set runner URL to port 8222. Got: {runner_url!r}"
...
class TestRunFallbackWarning:
    ...
    def test_runner_flag_missing_warns_or_errors(self):
        ...
        output_lower = (result.output or "").lower()
        has_actionable = any(
            kw in output_lower for kw in ("build", "runner", "warning", "warn", "not found", "missing", "error")
        )
        assert has_actionable or result.exit_code != 0, (
            f"Expected warning/error when runner not built. exit={result.exit_code} output={result.output!r}"
        )
    ...
    def test_run_help_mentions_runner_option(self):
        ...
        if "--runner" in (result.output or "") or "--use-runner" in (result.output or ""):
            ...
            pytest.skip("--runner flag not yet in 'specterqa ios run' help. Add it when implementing runner CLI.")
