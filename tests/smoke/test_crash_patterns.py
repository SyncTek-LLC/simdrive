...
# ---------------------------------------------------------------------------
# Dogfood Issue 1 (Palace-specific): State mutation + notification cascade
# ---------------------------------------------------------------------------

@requires_live
class TestPalaceNotificationCascade:
    """Reproduces the Palace crash: state-mutating operations fire rapid
    NotificationCenter posts that crash XCTest's debug logging.

    The PalacePatternTab simulates borrow/download/return/library-switch
    with the same notification patterns as Palace.
    """
    ...
    def test_borrow_flow_survives(self):
        ...
        _assert_runner_alive("Palace borrow flow")

        # Verify state changed
        el = _find(identifier="palace_book_state")
        assert el is not None, "palace_book_state not found"
    ...
    def test_download_flow_survives(self):
        ...
        _tap(identifier="palace_btn_download")
        time.sleep(3.0)  # wait for simulated download (20 progress ticks × 100ms)

        _assert_runner_alive("Palace download flow")
    ...
    def test_return_flow_survives(self):
        ...
        _assert_runner_alive("Palace return flow")
    ...
    def test_notification_flood_10_rapid_survives(self):
        ...
        el = _find(identifier="palace_notification_count")
        assert el is not None
        val = str(el.get("label", "") or el.get("value", ""))
    ...
    def test_library_switch_modal_survives(self):
        ...
        _assert_runner_alive("Palace library switch modal")
        ...
        _tap(identifier="palace_library_0")
        time.sleep(2.0)

        _assert_runner_alive("Palace library selection + notification cascade")
...
class TestXCTestCrashMitigation:
    ...
    in Palace dogfood: sheet presentation + notification cascade +
