...
def test_pbxproj_has_generate_infoplist_file():
    ...
    assert count >= 4, (
        f"Expected GENERATE_INFOPLIST_FILE in all 4 buildSettings blocks, found {count}"
    )
...
def test_project_injector_uses_runner_project():
    ...
    src = (
        REPO_ROOT / "src" / "specterqa" / "ios" / "project_injector.py"
    ).read_text()
    assert "SpecterQARunner.xcodeproj" in src, (
        "build() must reference SpecterQARunner.xcodeproj"
    )
    assert '"SpecterQARunner"' in src, (
        'build() must use scheme "SpecterQARunner"'
    )
