...
- TestKitApp with 5 tabs (Form, List, Nav, Stress, Example Reader patterns)
...
### Priority 4: Example Reader dogfood

Example Reader source is at `/Users/atlas/Downloads/ios-core-modernize-whole-shot/` but can't compile on this machine (missing Carthage artifacts). Either:
- Get a pre-built `.app` from the Example Reader team
- Run `carthage bootstrap` to fetch AudioEngine.xcframework
- Test on the other machine where Example Reader is already installed
...
| `tests/smoke/test_crash_patterns.py` | 27 crash pattern + Example Reader + mitigation tests |
| `TestKitApp/` | 5-tab test app (Form, List, Nav, Stress, Example Reader patterns) |
...
- v12.6.0: Example ReaderPatternTab (notification cascade, Combine progress, UIKit modal)
