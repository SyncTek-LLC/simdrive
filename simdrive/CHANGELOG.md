...
"Where am I?", toolbar toggles, and the announcements they trigger):

- **`perform_accessibility_action`** — fire a `UIAccessibilityCustomAction` by
  name (e.g. "Where am I?", "Next page"). These are normally reachable only via
...
- 6 new tests in `tests/test_b4_palace_dogfood_fixes.py` — one per fix above + happy-path counterpart
