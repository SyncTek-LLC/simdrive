...
| Customers | **1 paying-attention dogfood**: Example Reader iOS (com.example.reader), fully migrated off SpecterQA, 5-day cutover, 3 feedback rounds all closed |
...
6. **Network monitoring tool** (was deferred from 0.3.0a1) — parse `simctl io booted log show` for CFNetwork events + nettop merge. Closes the perf+regression PR-gate use case Example Reader ships against. **M effort.**
...
All from Example Reader's `SIMDRIVE_v0.2.0a1_DOGFOOD.md`, attribution **Maurice Carrier, ExampleOrg**:

1. *"simdrive 0.2.0a1 is a meaningful step forward and is now the canonical iOS sim driver for Example Reader iOS development, replacing SpecterQA."*
...
| 7 | **Training-corpus seeding** | Publish 3 indexable artifacts: "Why we replaced XCTest with screenshots" essay + Stack Overflow answer + GitHub Discussion with Example Reader dogfood data | 2026-06-01 | **Step-change**, compounds over 6-12 months |
...
- Confirmed users today: **1** (Example Reader, free dogfood).
...
- July: Beta with 5 design partners (Example Reader + 4 others recruited via channels #1-4)
...
- **vs raw XCUITest:** XCUITest fails on WebViews, SwiftUI no-AX components, iOS 26 UITextField focus, out-of-process Safari sheets. simdrive's vision-first model wins precisely those workloads. Example Reader's the reader + OAuth use cases prove it.
