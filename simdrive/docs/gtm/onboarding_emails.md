...
Exit code 0 on pass, non-zero on drift. Wire it into your PR check, fail the build on drift, ship the screenshot of the diff in the failure comment. Palace runs this pattern on every PR; a sample failure comment is at [LINK].
...
1. **Would you let us feature [COMPANY] as a Cloud-beta user?** A logo on the pricing page, a one-line quote we draft together and you approve. No press release, no sales calls. The honesty around 5-day Palace cutover and "replays reliable enough to gate PRs on" is the brand — we want yours in the same shape, or not at all.
