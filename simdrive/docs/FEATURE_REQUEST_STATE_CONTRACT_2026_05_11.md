...
**Filer:** Palace iOS dogfood (Maurice Carrier)
**Date:** 2026-05-11
...
Goal: smoke-test a Palace iOS PR that refactored the SAML auth surface. Plan: replay the only SAML recording (`pr907-saml-signin-gorgon`, 23 steps) against the post-refactor build.
...
2. **On verification failure**: halt with a structured error:
   ```json
   {
     ...
     "expected": { "text_subset_required": ["Add Library", "Palace Bookshelf"] },
     "actual":   { "text_subset_present": ["Allow Notifications", "Don't Allow", "Palace would like to send"] },
...
Palace landed a complementary primitive layer in https://github.com/ThePalaceProject/ios-core/pull/937 — `.palace-state/operations/reset-fresh-install.sh` + snapshot/restore. With Layer 2 (this proposal) shipped:

- Palace's recording authors run `simctl privacy grant` + reset-fresh-install before `record_start`
...
- `verify-pr.sh` lints that every Palace recording has a non-empty `requires:` (Palace's Layer 3)
...
**Palace context**: https://github.com/ThePalaceProject/ios-core/pull/937 (Layer 1, just landed for review)
