...
- [ ] **Read open issues** for the previous release tag. The Palace team logs dogfood-found bugs against the current beta. Surface anything filed in the last 7 days; address blockers or document deferrals in the release notes.
...
## What we learned (Palace b3 dogfood post-mortem, 2026-05-22)
...
- **F-B3-009** `clear_field` did not emit a recording step → silent divergence on replay. **Fix:** added `_record_act_step` calls on both branches; new test pins this in `test_b4_palace_dogfood_fixes.py`.
