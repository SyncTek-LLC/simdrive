...
- **F-B3-009** `clear_field` did not emit a recording step → silent divergence on replay. **Fix:** added `_record_act_step` calls on both branches; new test pins this in `test_b4_Example Reader_dogfood_fixes.py`.
