...
> "SpecterQA is now the canonical iOS sim driver for Palace iOS development, replacing the predecessor. The single biggest reason it was failing — the path that broke `UITextField` focus — is fully fixed."
>
> — Maurice Carrier, ThePalaceProject (Palace iOS, [`org.thepalaceproject.palace`](https://thepalaceproject.org))
