...
| `docs/marketing/case_study_example.md` | Bulk find/replace | "SpecterQA" → "SimDrive" everywhere except the historical references to "the predecessor" — those become **"the SpecterQA browser product"** to disambiguate. The package versions (`specterqa-ios 0.2.0a1` → `simdrive 0.2.0a1`) revert to their `simdrive` namespace. |
...
- [Case study: Example Reader iOS migrated in 5 days](/blog/case-study-example-simdrive) — the reader + OAuth coverage that XCTest couldn't reach
...
[Start your 14-day trial →](https://synctek.io/simdrive/trial) · [Read the docs](https://synctek.io/docs/simdrive) · [Case study: Example Reader iOS](https://synctek.io/blog/case-study-example-simdrive)
...
>
> Three dogfood rounds, all feedback closed. Five days, sim driver fully cut over. The flow that finally proved it: a `WKWebView` reading regression XCUITest couldn't see, gated on a SimDrive replay. Full case study: synctek.io/blog/case-study-example-simdrive. Worth 4 minutes if you're evaluating.
