...
| Vision-first | OCR + Set-of-Mark + stable_id + stable_id_loose + confidence_band | `observe.py` + `som.py` | unit tests + Palace verbatim quote |
| Input | Real UITouch HID via CoreSimulator (iOS 26 TextField focus works) | `simdrive/native/src/simdrive_input.m` | live E2E + Palace verbatim quote |
| Record/replay | YAML + stable_id resolution + SSIM region masking | `recorder.py` | Palace: *"replays now reliable enough to gate PRs on"* |
...
**Customers:** Palace iOS (ThePalaceProject) — migrated off the predecessor in 5 days, three dogfood rounds, all feedback closed, replays now gate their PRs.
...
**29 MCP tools stay user-facing.** Demoted in *docs* (the journey runner is the headline workflow), not in *visibility*. Palace's validation flowed through them directly; hiding them burns evidence before the journey runner has independent proof.
...
| 7-8 | Component 9 (hardening) | Component 9 (hardening) | All 9 components green; Palace journey corpus passes |
...
3. **Journey YAML schema lock-in** — wrong schema in 1.0 breaks every customer's journeys in 1.1. Mitigation: `schema_version: 1` reservation + 2-design-partner draft cycle (Palace + 1 other) before public cut.
...
| 5 | **MCP primitives stay user-facing or become internal** | **User-facing.** Palace's validation flowed through them; hiding them burns evidence before journey runner has its own proof. | Disagrees with chairman's stated direction. |
...
5. **Design-partner schema review** — Palace + 1 other for journey YAML schema before lock.
