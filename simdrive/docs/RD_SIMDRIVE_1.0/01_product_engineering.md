...
- **Pro of keeping (what I recommend):** Palace dogfood proved the MCP surface
  has organic agent demand outside our journey runner — Atlas drives Palace
...
3. **The journey YAML schema is wrong on the first try.** This is the only
   ...
   partners (Palace + one TBD) before the public 1.0 cut, and treat journey
...
2. **The 29 MCP tools should remain user-facing, not be made internal.** The
   ...
   them in *documentation*, not in *visibility*. Reasoning: Palace dogfood is
   today's only paying-attention validation, and Palace consumes the MCP
