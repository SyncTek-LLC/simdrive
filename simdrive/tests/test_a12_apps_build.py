...
def test_apps_response_includes_build_field_on_device(monkeypatch):
            ...
            "apps": [
                {
                    ...
                    "version": "3.1.0",
                    "bundleVersion": "789",
                    "url": "file:///private/var/containers/Bundle/com.example.reader/",
                },
                {
    ...
    apps_by_id = {a["bundle_id"]: a for a in apps}
    assert apps_by_id.get("com.example.reader", {}).get("build") == "789", (
        f"Expected build='789' for com.example.reader, got: "
        f"{apps_by_id.get('com.example.reader', {}).get('build')!r}"
    )
