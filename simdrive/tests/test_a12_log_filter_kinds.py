...
def test_logs_predicate_kind_substring_post_filters_on_device(tmp_path, monkeypatch):
    ...
    assert result.get("ok") is True, f"Expected ok=True, got: {result}"
    returned_lines = [ln for ln in result.get("logs", "").splitlines() if ln]
    Example Reader_lines = [ln for ln in returned_lines if "Example Reader" in ln]
    assert len(Example Reader_lines) == 2, (
        f"Expected 2 lines containing 'Example Reader', got {len(Example Reader_lines)}: {returned_lines}"
    )
