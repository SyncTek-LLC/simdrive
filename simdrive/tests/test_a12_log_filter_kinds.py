...
Tests:
  ...
  3. test_logs_predicate_kind_substring_post_filters_on_device
     - device + predicate_kind="substring", predicate="Palace".
       Mock 5 lines, 2 contain "Palace". Assert result has those 2.
...
def test_logs_predicate_kind_nspredicate_on_sim_uses_log_show(tmp_path, monkeypatch):
    ...
    result = server_mod.tool_logs({
        "session_id": sid,
        "lines": 50,
        "predicate": "eventMessage CONTAINS 'Palace'",
...
def test_logs_predicate_kind_substring_post_filters_on_device(tmp_path, monkeypatch):
    """device + predicate_kind='substring', predicate='Palace' → 2 Palace lines returned.

    5 fake lines, 2 contain 'Palace'. Result must have exactly those 2.
    ...
    assert result.get("ok") is True, f"Expected ok=True, got: {result}"
    returned_lines = [ln for ln in result.get("logs", "").splitlines() if ln]
    palace_lines = [ln for ln in returned_lines if "Palace" in ln]
    assert len(palace_lines) == 2, (
        f"Expected 2 lines containing 'Palace', got {len(palace_lines)}: {returned_lines}"
    )
    others = [ln for ln in returned_lines if "Palace" not in ln]
    assert not others, (
        f"substring filter should exclude non-Palace lines, but got: {others}"
    )
