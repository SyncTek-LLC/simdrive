...
- 5 new MCP tools for the AI debugging loop:
  - `ios_app_relaunch(bundle_id, app_path?)` — reinstall/relaunch user app without tearing down runner (<2s without path, ~15s with).
  - `ios_logs_tail(since_last_call, filters…)` — incremental log stream with per-session cursor.
  - `ios_capture_state(include?)` — bundles screenshot + elements + logs + app_state + perf.
  - `ios_action_with_logs(action, log_window_ms)` — atomic: action + logs fired during it.
  - `ios_promote_session_to_test(name, path?)` — saves session as replay YAML + auto-validates; in-repo `./replays/` default.
- `runner/__init__.py` — `runner/` is now a proper Python package.
- Two E2E CI dogfood tests — CI replay workflow + AI debugging workflow.
- `rm -rf build dist` step in publish workflow.

### Changed

- `session_manager._deploy_runner` delegates to `RunnerProcess`.
- `_runner_source_dir()` uses `importlib.resources.files('runner')` as primary resolution.
- MCP tool count: 41 → 43 (net; removed 3 and added 5).
- `pyproject.toml` uses `packages.find` (replaces long package-data glob list).
- `MANIFEST.in` simplified (no more `runner_source` mirror).
- `setup.py` slimmed to a minimal shim (build_py override removed).

### Removed — BREAKING

- `ios_start_runner` — use `ios_start_session(backend="xctest")` instead.
- `ios_stop_runner` — use `ios_stop_session()` instead.
...
- `ios_promote_session_to_test` sanitizes `name` and resolves `path=` against `Path.cwd()`.

### Migration

- Replace `ios_start_runner` → `ios_start_session(backend="xctest")`.
- Replace `ios_stop_runner` → `ios_stop_session()`.
- Replace `ios_save_replay(name)` → `ios_stop_recording(name=name)`.
