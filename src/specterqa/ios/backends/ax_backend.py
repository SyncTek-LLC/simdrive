...
class AXBackend:
    ...
    def _walk_sibling_windows(self, results: list[dict], limit: int) -> None:
        ...
        Example Reader dogfood Issue 2 fix (2026-04-17).
    ...
    # ------------------------------------------------------------------
    # SpringBoard alert handling (Example Reader dogfood Issue 3)
    # ------------------------------------------------------------------
    ...
    def tap(
        ...
        if label is not None or identifier is not None:
            ...
            # the Example Reader view is requested).  Navigate back once and retry.
            back_btn = next(
                (
                    e for e in els
